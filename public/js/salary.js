...
$(document).ready(function () {
    ...
    $("#salary-form").on("input change", "input, select", calculate);
    calculate();

    $("#salary-form").on("submit", function (e) {
        e.preventDefault();
        $saveSalary.html("Saving...");
        postJSON(API, payload())
            .done((res) => {
                const data = res.data ?? res.original?.data ?? res;
...
$(function () {
    ...
    function renderHistoryItem(row) {
        ...
        const $li = $("<li/>", {
            class: "group rounded-xl border border-slate-200 dark:border-slate-700 bg-white/90 dark:bg-slate-800 p-3 mb-2 shadow-sm transition transform opacity-0 translate-y-2",
    ...
    <div class="flex items-center justify-between mb-2">
        <div class="text-sm font-semibold text-slate-900 dark:text-slate-100">
            $${gross} <span class="text-slate-400 mx-1">→</span> $${net}
        </div>
    ...
    `;
        ...
        // Click to restore into form (optional)
